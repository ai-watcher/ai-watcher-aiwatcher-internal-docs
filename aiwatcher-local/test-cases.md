...
- `S-32` Watch - [Watch signals reach the developer without manual CLI polling](#s-32): Done: `watch --notify` fires a local OS notification with a dashboard deep link on context/runway/loop/velocity pressure, click-through opens the session review drawer, and every firing is persisted and shown under hook-status. Still missing: tray/menu-bar item and editor-panel surfacing.
- `S-33` Watch - [Runtime hygiene identifies stale local AI runtimes](#s-33): Done: `aiwatcher processes` (with --stale-only, --json) ships in main with exactly the read-only field list this scenario expects. Still missing: dashboard UI surfacing -- CLI-only today.
