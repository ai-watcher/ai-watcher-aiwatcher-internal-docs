...
| Claude Desktop Code tab | Claude Code hook path when the Desktop Code tab exposes the same hook runtime | Hard gate only when hook-status proves invocation; otherwise companion/manual fallback | To verify | Submit a risky prompt in Desktop Code tab and confirm hook-status records that exact surface. Recheck after Desktop updates; do not infer from Claude Code CLI success. |
...
| Native Companion / local overlay | Small draggable Companion plus dashboard Console tied to durable intervention records | Plan, Scan, Fresh Start, prompt-gate review, focused continuation, inspect loop, skip/continue, and receipt recording depending on signal and identity confidence | To verify | Run S-32/S-44/S-45/S-46. Confirm Companion is quiet when healthy, highlights one relevant action, protects clipboard on Fresh Start, and clears attention after skip/continue/copy/viewed receipt. |
