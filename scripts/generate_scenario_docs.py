#!/usr/bin/env python3
"""Generate AIWatcher scenario docs from a product scenarios.json file.
...
PRODUCT_LABEL = os.environ.get("AIWATCHER_PRODUCT_LABEL", "AIWatcher Local")
REVIEW_HOME_TITLE = os.environ.get("AIWATCHER_REVIEW_HOME_TITLE", f"{PRODUCT_LABEL} Review Home")
PLATFORM_GUIDANCE = os.environ.get(
    "AIWATCHER_PLATFORM_GUIDANCE",
...
def section_nav() -> str:
    ...
    links.extend(_extra_links("AIWATCHER_EXTRA_NAV_JSON"))
    return " · ".join(f"[{label}]({href})" for label, href in links)


def _extra_links(env_name: str) -> list[tuple[str, str]]:
    raw = os.environ.get(env_name, "").strip()
    if not raw:
        return []
    items = json.loads(raw)
    return [(str(item["label"]), str(item["href"])) for item in items]


def _extra_review_sections() -> list[dict[str, str]]:
    raw = os.environ.get("AIWATCHER_EXTRA_REVIEW_SECTIONS_JSON", "").strip()
    if not raw:
        return []
    return [
        {
...
def _review_sections(data: dict[str, Any]) -> list[tuple[str, str, str]]:
    raw = os.environ.get("AIWATCHER_REVIEW_SECTIONS_JSON", "").strip()
    if raw:
        return [
            (str(item["section"]), str(item["href"]), str(item["use"]))
            for item in json.loads(raw)
        ]
    if data.get("scope", {}).get("enterprise_boundary"):
        return [
...
def _implementation_order() -> list[str]:
    raw = os.environ.get("AIWATCHER_IMPLEMENTATION_ORDER_JSON", "").strip()
    if not raw:
        return []
    return [str(item) for item in json.loads(raw)]


def _review_priority_limit() -> int:
    raw = os.environ.get("AIWATCHER_REVIEW_PRIORITY_LIMIT", "10").strip()
...
def render_review_home(data: dict[str, Any]) -> str:
    ...
    priority = sorted(open_items, key=lambda s: (STATUS_ORDER.get(s.get("status", ""), 9), s.get("id", "")))[
        : _review_priority_limit()
    ]
    lines = [
        f"# {REVIEW_HOME_TITLE}",
        "",
    ...
    for section, href, use in _review_sections(data):
        lines.append(f"| [{section}]({href}) | {use} |")
    for section in _extra_review_sections():
        lines.append(f"| [{section['section']}]({section.get('href', section['section'])}) | {section['use']} |")
    implementation_order = _implementation_order()
    if implementation_order:
        lines.extend(["", "## Recommended Implementation Order", ""])
        lines.extend(f"{index}. {item}" for index, item in enumerate(implementation_order, start=1))
    lines.extend([
        "",
        "## Interactive HTML",
        "",
        "`index.html` is still generated for the tabbed browser experience. "
        "GitHub displays HTML files as source, so use these Markdown pages for normal GitHub review.",
        "",
        f"Generated from `{SOURCE.relative_to(ROOT) if SOURCE.is_relative_to(ROOT) else SOURCE}`. The JSON is the private source of truth; "
...
def render_scope_markdown(data: dict[str, Any]) -> str:
    ...
    if scope.get("enterprise_boundary"):
        lines.extend(["## Enterprise Boundary", "", scope.get("enterprise_boundary", ""), ""])
    if scope.get("oss_dependency"):
        lines.extend(["## OSS Dependency", "", scope.get("oss_dependency", ""), ""])
    if scope.get("oss_boundary"):
        lines.extend(["## OSS Boundary", "", scope.get("oss_boundary", ""), ""])
    lines.extend(["## Strategic Filter", ""])
    for item in scope.get("strategic_filter", []):
        lines.append(f"- {item}")
    not_scope_heading = "Not In Scope" if scope.get("enterprise_boundary") else "Not In OSS Scope"
    lines.extend(["", f"## {not_scope_heading}", ""])
...
def render_requirements_markdown(data: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Lifecycle Coverage", "", *render_lifecycle_counts(data), ""])
    return "\n".join(lines)
...
def render_test_cases_markdown(data: dict[str, Any]) -> str:
    ...
    example_value_key = "buyer_value" if any("buyer_value" in example for example in data.get("examples", [])) else "feeling"
    example_value_label = "Buyer value" if example_value_key == "buyer_value" else "Expected feeling"
    lines = [
        "# Test Cases",
        "",
        section_nav(),
    ...
    lines.extend([
        "## Concrete Examples",
        "",
        f"| Situation | AIWatcher response | {example_value_label} | Status |",
        "| --- | --- | --- | --- |",
    ])
    for example in data.get("examples", []):
        lines.append(
            f"| {md_cell(example.get('situation'))} | {md_cell(example.get('response'))} | "
            f"{md_cell(example.get(example_value_key))} | {status_text(example.get('status', ''))} |"
        )
...
<body>
<div class="wrap">
<header><div><div class="brand">{html.escape(PRODUCT_LABEL)}</div><h1>{html.escape(data['title'])}</h1><p class="sub">Generated from <code>{html.escape(str(SOURCE.relative_to(ROOT) if SOURCE.is_relative_to(ROOT) else SOURCE))}</code>. {html.escape(data.get('scope', {}).get('position', data.get('summary', '')))}</p><div class="meta">Updated {html.escape(data.get('updated',''))} · {len(data['scenarios'])} scenarios · generated by scripts/generate_scenario_docs.py</div></div><div class="thesis"><b>Testing principle</b><p>Done means verified behavior, not merely code presence. Unsupported platforms should be labeled honestly and routed to Prompt Companion, MCP, wrappers, or extensions.</p></div></header>
...
<nav class="tabs"><button class="tab active" data-tab="scope">Scope</button><button class="tab" data-tab="requirements">Requirements</button><button class="tab" data-tab="workflows">UX Workflows</button><button class="tab" data-tab="platforms">Platform Coverage</button><button class="tab" data-tab="tests">Test Cases</button><button class="tab" data-tab="gaps">Gaps</button><button class="tab" data-tab="decisions">Open Decisions</button></nav>
<section class="section active" id="scope"><h2>Scope and Position</h2><div class="grid two" id="scopeCards"></div><h2>Acceptance Rules</h2><div class="grid two" id="acceptanceCards"></div></section>
<section class="section" id="requirements"><h2>Lifecycle Requirements</h2><div class="lifecycle" id="lifecycleRail"></div><h2>Requirement Matrix</h2><table><thead><tr><th>Requirement</th><th>Lifecycle</th><th>User value</th><th>Status</th><th>Covered by</th></tr></thead><tbody id="requirementsRows"></tbody></table></section>
<section class="section" id="workflows"><h2>User Experience Workflows</h2><div class="grid two" id="workflowCards"></div><h2>Concrete Examples</h2><table><thead><tr><th>Situation</th><th>AIWatcher response</th><th>{html.escape(example_value_label)}</th><th>Status</th></tr></thead><tbody id="exampleRows"></tbody></table></section>
...
const phases=['Plan','Watch','Control','Prove','Improve','Failsafe'];
const exampleValueKey='{example_value_key}';
function esc(v){{return String(v??'').replace(/[&<>"']/g,ch=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[ch]));}}
function render(){{
 const scope=DATA.scope||{{}};
 const scopeItems=[['Position', scope.position]];
 if(scope.enterprise_boundary) scopeItems.push(['Enterprise boundary', scope.enterprise_boundary]);
 if(scope.oss_dependency) scopeItems.push(['OSS dependency', scope.oss_dependency]);
 if(scope.oss_boundary) scopeItems.push(['OSS boundary', scope.oss_boundary]);
 scopeItems.push(['Strategic filter', (scope.strategic_filter||[]).join('\\n')]);
 scopeItems.push([scope.enterprise_boundary?'Not in scope':'Not in OSS scope', (scope.not_scope||[]).join('\\n')]);
 document.getElementById('scopeCards').innerHTML=scopeItems.map(item=>`<div class="card"><div class="kicker">${{esc(item[0])}}</div><p>${{esc(item[1]||'')}}</p></div>`).join('');
 document.getElementById('acceptanceCards').innerHTML=(DATA.acceptance_rules||[]).map(rule=>`<div class="card"><div class="kicker">Acceptance rule</div><h3>${{esc(rule.title)}}</h3><p>${{esc(rule.body)}}</p></div>`).join('');
 ...
 document.getElementById('exampleRows').innerHTML=(DATA.examples||[]).map(e=>`<tr><td class="mono">${{esc(e.situation)}}</td><td>${{esc(e.response)}}</td><td>${{esc(e[exampleValueKey]??e.feeling??e.buyer_value)}}</td><td>${{statusLabel[e.status]||esc(e.status)}}</td></tr>`).join('');
 ...
 document.getElementById('decisionCards').innerHTML=(DATA.open_decisions||[]).map(d=>`<div class="card"><div class="kicker">${{esc(d.status||'open')}}</div><h3>${{esc(d.decision)}}</h3><p><b>Options:</b> ${{esc(d.options)}}</p><p><b>Recommendation:</b> ${{esc(d.recommendation)}}</p></div>`).join('');
